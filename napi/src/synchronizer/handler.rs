...
impl Synchronizer {
  ...
  pub fn status(&self, request: Buffer) -> Buffer {
    let r = mystiko_lib::synchronizer::status::<&[u8]>(request.as_ref()).encode_to_vec();
    Buffer::from(r)
  }

  #[napi]
  pub fn sync(&self, request: Buffer) -> Buffer {
    let r = mystiko_lib::synchronizer::sync::<&[u8]>(request.as_ref()).encode_to_vec();
    Buffer::from(r)
  }

  #[napi]
  pub fn reset(&self, request: Buffer) -> Buffer {
    let r = mystiko_lib::synchronizer::reset::<&[u8]>(request.as_ref()).encode_to_vec();
    Buffer::from(r)
  }
}
